...
def upload_pdf():
    current_app.logger.info("PDF upload request received.")
    ...
    try:
        ...
        if len(pdfs) == 0:
            db.session.add(pdf)
            current_app.logger.info(f"New PDF record created: {stored_filename}")
        else:
            current_app.logger.info(f"Existing PDF found, deleting old record for: {stored_filename}")
            delete_pdf(pdf_id=pdfs[0].id)
        db.session.commit()
        current_app.logger.info(f"PDF committed to database: {stored_filename}")
    except SQLAlchemyError as e:
        db.session.rollback()
        current_app.logger.error(f"Database error during PDF upload: {e}")
        abort(500, str(e))
    os.remove(tmp_path)
...
def list_pdfs():
    ...
    current_app.logger.info(f"Listing PDFs with filters - name: {name}, meta_key: {meta_key}, meta_value: {meta_value}")
    ...
    pdfs = query.all()
    current_app.logger.info(f"Found {len(pdfs)} PDFs matching filters.")
    return jsonify(pdf_schema.dump(pdfs, many=True))

@pdf_bp.route('/download/<int:pdf_id>', methods=['GET'])
def download_pdf(pdf_id):
    current_app.logger.info(f"Download requested for PDF ID: {pdf_id}")
    ...
    current_app.logger.info(f"PDF downloaded to temporary path: {tmp_path}")
    return send_file(tmp_path, as_attachment=True, download_name=pdf.original_filename)

@pdf_bp.route('/<int:pdf_id>', methods=['GET'])
def get_pdf(pdf_id):
    current_app.logger.info(f"Fetching metadata for PDF ID: {pdf_id}")
    pdf = PDF.query.get_or_404(pdf_id)
    return jsonify(pdf_schema.dump(pdf))

@pdf_bp.route('/<int:pdf_id>', methods=['DELETE'])
def delete_pdf(pdf_id):
    current_app.logger.info(f"Delete requested for PDF ID: {pdf_id}")
    pdf = PDF.query.get_or_404(pdf_id)
    try:
        storage_dir = f"{current_app.config['PARENT_FOLDER']}/{pdf.original_filename.split('.')[0]}"
        file_manager.delete_directory(storage_dir)
        current_app.logger.info(f"PDF file deleted from storage: {storage_dir}")
        db.session.delete(pdf)
        db.session.commit()
        current_app.logger.info(f"PDF record deleted from database: {pdf_id}")
    except SQLAlchemyError as e:
        db.session.rollback()
        current_app.logger.error(f"SQLAlchemy error during PDF deletion: {e}")
        abort(500, str(e))
    except Exception as e:
        current_app.logger.error(f"File deletion failed for PDF {pdf_id}: {e}")

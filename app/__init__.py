...
app.logger.info('Document Manager startup')
app.logger.info(f'Creating base directories')
if not os.path.exists(Config.TMP_FOLDER):
    os.mkdir(Config.TMP_FOLDER)
    app.logger.info(f'Created temporary folder: {Config.TMP_FOLDER}')
if not os.path.exists(Config.STATIC_FOLDER):
    os.mkdir(Config.STATIC_FOLDER)
    app.logger.info(f'Created static folder: {Config.STATIC_FOLDER}')
...
try:
    ...
    app.logger.error(f'Error creating parent folder in file manager: {e}')
    raise e
db.init_app(app)
app.logger.info('Initialized SQLAlchemy database')
ma.init_app(app)
app.logger.info('Initialized Marshmallow')
...
app.logger.info('Registered blueprint: pdf_bp with prefix /api/pdfs')
app.register_blueprint(attachment_bp, url_prefix='/api/attachments')
app.logger.info('Registered blueprint: attachment_bp with prefix /api/attachments')
...
app.logger.info('Registered blueprint: ui_bp')
